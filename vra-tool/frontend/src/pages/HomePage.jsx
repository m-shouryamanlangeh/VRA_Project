...
export default function HomePage() {
  ...
  // Whether the backend requires an LLM API key. In deterministic mode
  // (USE_LLM=false) no Gemini key is needed, so the "missing key" banner
  // must not show. Default true until /health responds (fail safe).
  ...
  useEffect(() => {
    ...
    apiFetch("/health")
      ...
      .then((d) => {
        if (active && d && typeof d.use_llm === "boolean") setLlmRequired(d.use_llm);
      })
      .catch(() => {});
    return () => {
      active = false;
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const pdfHref = data.pdf_url ? apiUrl(data.pdf_url) : "";
  ...
  const showKeyWarning = llmRequired && !hasGeminiKey;
    ...
    <main className="max-w-3xl mx-auto px-6 py-10">
      ...
      {!llmRequired && (
        <div className="mb-6 p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-900 text-sm">
          <strong>Deterministic mode.</strong> Reports are generated by the
          rules engine from OSINT collectors — no LLM and no API key required.
        </div>
      )}
