...
class AppSettings(BaseSettings):
    ...
    # Fully deterministic, LLM-free mode (the default). When False, NO Gemini /
    # OpenRouter / any external LLM call is made during report generation: the
    # pipeline runs collectors → deterministic risk framework (app.core.risk) →
    # templated narrative (app.core.narrative) → PDF, and requires no API key at
    # all. Every score, recommendation and sentence is traceable to cited
    # evidence. Set USE_LLM=true only to opt into the hybrid LLM-assisted path
    # (which then requires a Gemini/OpenRouter API key).
    USE_LLM: bool = False
